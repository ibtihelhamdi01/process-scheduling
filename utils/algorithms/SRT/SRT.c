...
ExecutedTask* srt_wrapper(process *proc, int proc_count, int *executed_count, options ops) {
    return get_srt_output(proc, proc_count, executed_count);
}
AlgorithmInfo* get_algorithm_info_plugin(void) {
    static AlgorithmInfo info = {
        .name = "SRT",
        .display_name = "Shortest Remaining Time",
        .id = SRT,
        .execute = srt_wrapper,
