...
ExecutedTask* priority_np_wrapper(process *proc, int proc_count, int *executed_count, options ops) {
    return get_priority_np_output(proc, proc_count, executed_count);}
AlgorithmInfo* get_algorithm_info_plugin(void) {
    static AlgorithmInfo info = {
        .name = "PRIORITY_NP",
        .display_name = "Priority (Non-Preemptive)",
        .id = PRIORITY,
        .execute = priority_np_wrapper,
