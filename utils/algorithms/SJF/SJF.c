...
ExecutedTask* sjf_wrapper(process *proc, int proc_count, int *executed_count, options ops) {
    return get_sjf_output(proc, proc_count, executed_count);
}
AlgorithmInfo* get_algorithm_info_plugin(void) {
    static AlgorithmInfo info = {
        .name = "SJF",
        .display_name = "Shortest Job First",
        .id = SJF,
        .execute = sjf_wrapper,
