...
ExecutedTask* priority_wrapper(process *proc, int proc_count, int *executed_count, options ops) {
    return get_priority_output(proc, proc_count, executed_count);
}

AlgorithmInfo* get_algorithm_info_plugin(void) {
    static AlgorithmInfo info = {
        .name = "PRIORITY_P",
        .display_name = "Priority (Preemptive)",
        .id = PRIORITY_P,
        .execute = priority_wrapper,
